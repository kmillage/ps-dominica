...
last_positions_sql <- paste0("WITH 
  ...
  trip_table AS (
    SELECT
      trip_id,
      scenario,
      first_timestamp_in_reserve
    FROM
      `emlab-gcp.dominica.trips_in_summary_table`
    WHERE
  ...
  points_before_inside AS(
      ...
      JOIN trip_table as tp
        ...
        AND ap.timestamp < tp.first_timestamp_in_reserve
  ),
  ...
  last_point_before_inside AS(
      SELECT
        trip_id,
        scenario,
        timestamp as last_timestamp_before_reserve,
        lat as last_lat_before_reserve,
        lon as last_lon_before_reserve
      FROM (
...
FROM
  `emlab-gcp.dominica.trips_in_summary_table`
LEFT JOIN all_trip_details  USING(trip_id, scenario, first_timestamp_in_reserve)
ORDER BY
  trip_id,
  scenario"
...
first_positions_sql <- paste0("WITH 
  ...
  trip_table AS (
    SELECT
      trip_id,
      scenario,
      last_timestamp_in_reserve
    FROM
      `emlab-gcp.dominica.trips_out_summary_table`
    WHERE
      scenario = ", scenarios[i], "
  ),
  ...
  points_after_inside AS(
      ...
      JOIN trip_table as tp
        ON ap.trip_id = tp.trip_id
        AND ap.timestamp > tp.last_timestamp_in_reserve
  ),
  ...
  last_point_after_inside AS(
      SELECT
        trip_id,
        scenario,
        timestamp as first_timestamp_after_reserve,
        lat as first_lat_after_reserve,
        lon as first_lon_after_reserve
      FROM (
...
LEFT JOIN last_point_after_inside USING(trip_id, scenario)
ORDER BY
  trip_id,
  scenario")
...
FROM
  `emlab-gcp.dominica.trips_out_summary_table`
LEFT JOIN all_trip_details  USING(trip_id, scenario, last_timestamp_in_reserve)
ORDER BY
  trip_id,
  scenario"
