...
summary_by_year_outside <- where_be_whales_outside %>%
  mutate(name = ifelse(is.na(name), "Other areas", name)) %>%
  st_drop_geometry() %>%
  group_by(year, name) %>%
  summarize(n_sightings = n_distinct(obs),
            n_adults = sum(adults, na.rm = T),
...
# Explore results excluding Roseau hotspot 

There's a major concentration of sightings right by Roseau... Like an order of magnitude greater than most other areas. This seems suspicious. 

```{r}
# Bin sightings into lat/lon bins
whales_recent_binned <- whales_recent %>%
  mutate(lat_bin = (floor(lat/0.01)*0.01) + 0.005,
         lon_bin = (floor(lon/0.01)*0.01) + 0.005) %>%
  group_by(lon_bin, lat_bin) %>%
  summarize(count = n())

# Which bins have more than 200 sightings
potential_bad_whales <- whales_recent_binned %>%
  dplyr::filter(count > 200) %>%
  distinct(lat_bin, lon_bin)

# Coordinates to filter by 
lon_lim <- c(potential_bad_whales$lon_bin-0.005, potential_bad_whales$lon_bin+0.005)
lat_lim <- c(potential_bad_whales$lat_bin-0.005, potential_bad_whales$lat_bin+0.005)

# Now let's get those bad entries
bad_whales <- whales_recent %>%
  dplyr::filter(lat >= lat_lim[1] & lat <= lat_lim[2] & lon >= lon_lim[1] & lon <= lon_lim[2])

# Plot to check that we've got the right ones - Yep
bad_whales_plot <- ggplot()+
  geom_sf(data = bad_whales, size = 0.02, fill = "black")+
  geom_sf(data = dominica_sf_moll, aes(group = name), color = "black", fill = "grey", size = 0.2)+
  geom_sf(data = reserve_boundaries_sf, aes(color = rev(scenario_name)), alpha = 1, fill = NA, size = 0.5)+
  coord_sf(xlim = plot_bbox$xlim, ylim = plot_bbox$ylim)+
  theme_basic()+
...
Now let's recreate our results if we remove those whales... 

# Results - Version 2

This version has the whales right off of Roseau removed

## Find whale sightings inside the reserve area(s)

```{r}
# Good whales
good_whales_recent <- whales_recent %>%
  dplyr::filter(!obs %in% bad_whales$obs)

# Determine which reserve(s) each observation would fall within
good_where_be_whales <- st_join(good_whales_recent, reserve_boundaries_sf)

# Get sightings inside reserve
good_reserve_whales <- good_where_be_whales %>%
  dplyr::filter(!is.na(scenario))

# Get all other sightings
good_outside_whales <- good_where_be_whales %>%
  dplyr::filter(is.na(scenario))
```
...
```{r}
# Get totals by year
good_totals_by_year <- good_whales_recent %>%
  st_drop_geometry() %>%
  group_by(year) %>%
  summarize(n_sightings_total = n_distinct(obs),
            n_adults_total = sum(adults, na.rm = T),
...
# Summarize by year and reserve design
good_summary_by_year <- good_reserve_whales %>%
  st_drop_geometry() %>%
  group_by(year, scenario, scenario_name, area_km2) %>%
  summarize(n_sightings = n_distinct(obs),
            n_adults = sum(adults, na.rm = T),
            ...
            n_whales = sum(total, na.rm = T)) %>%
  ungroup() %>%
  left_join(good_totals_by_year, by = "year") %>%
  mutate(prop_sightings = n_sightings/n_sightings_total,
         prop_adults = n_adults/n_adults_total,
         prop_calves = n_calves/n_calves_total,
         prop_whales = n_whales/n_whales_total) %>%
...
```{r}
good_out_table <- good_summary_by_year %>%
  dplyr::filter(obs_type == "sightings") %>%
  distinct(scenario, year, value) %>%
  mutate(value = round(value*100, 2)) %>%
  group_by(scenario) %>%
...
```{r}
# Determine which other areas each observation would fall within
good_where_be_whales_outside <- st_join(good_outside_whales, other_areas_sf)

# Summarize by year and reserve design
good_summary_by_year_outside <- good_where_be_whales_outside %>%
  mutate(name = ifelse(is.na(name), "Other areas", name)) %>%
  st_drop_geometry() %>%
  group_by(year, name) %>%
  summarize(n_sightings = n_distinct(obs),
...
```{r}
# Make summary table
good_other_table <- good_summary_by_year_outside %>%
  dplyr::filter(obs_type == "sightings") %>%
  distinct(name, year, value) %>%
  mutate(value = round(value*100, 2)) %>%
  group_by(name) %>%
