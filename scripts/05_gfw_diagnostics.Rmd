...
dom_voyages <- read_csv(file.path(prj_cachalote_dir, "data", "processed", "dominica_voyages_gfw_2017_2022.csv")) # All voyages - 23,983
trips_in <- read_csv(file.path(prj_cachalote_dir, "data", "processed", "trips_in_summary_table_v2.csv"))
trips_out <- read_csv(file.path(prj_cachalote_dir, "data", "processed", "trips_out_summary_table_v2.csv"))
...
# Good inbound trips
inbound_voyages_good <- trips_in %>% 
  dplyr::filter(!is.na(lat_start) & !is.na(lon_start) & !is.na(lat_end) & !is.na(lon_end)) %>%
...
# Good outbound trips
outbound_voyages_good <- trips_out %>% 
  dplyr::filter(!is.na(lat_start) & !is.na(lon_start) & !is.na(lat_end) & !is.na(lon_end)) %>%
  dplyr::filter(!is.na(scenario)) %>%
  distinct(trip_id) %>% pull()
...
```{r}
# Calculate distance for our good trips
...
bad_trips_in <- trips_in_dist %>%
  dplyr::filter(dist_from_port > 5) %>%
  pull(trip_id)

# Percentage of trips being excluded
percent_bad_in <- length(bad_trips_in)/length(inbound_voyages_good)
trips_remaining_in <- length(inbound_voyages_good) - length(bad_trips_in)
```
...
trips_in_dist_plot <- trips_in_dist %>%
  ...
  theme_basic()+
  labs(x = "Distance between last AIS position and anchorage (km, red) or anchorage centroid (km, blue)", y = "Vessel type", 
       title = paste0("Inbound trips: Last AIS position relative to anchorage coordinates (N = ",  format(trips_remaining_in, big.mark = ","), ")"))
...
# Percentage of trips being excluded
percent_bad_out <- length(bad_trips_out)/length(outbound_voyages_good)
trips_remaining_out <- length(outbound_voyages_good) - length(bad_trips_out)
```
...
trips_out_dist_plot <- trips_out_dist %>%
  ...
  geom_density_ridges(aes(x = dist_from_port, y = vessel_type), alpha = 0.5, fill = "red")+
  theme_basic()+
  labs(x = "Distance between first AIS position and anchorage (km, red) or anchorage centroid (km, blue)", y = "Vessel type", 
       title = paste0("Outbound trips: First AIS position relative to anchorage coordinates (N = ",  format(trips_remaining_out, big.mark = ","), ")"))
...
# Make final list of good trips

```{r}
good_trips <- c(inbound_voyages_good[!inbound_voyages_good %in% bad_trips_in],
                outbound_voyages_good[!outbound_voyages_good %in% bad_trips_out])

save(good_trips,
     file = file.path(prj_cachalote_dir, "data", "processed", "good_trips.Rdata"))
```

# Vessels with a matching category

```{r}
trips_in_category <- trips_in %>%
  dplyr::filter(trip_id %in% good_trips) %>%
  distinct(trip_id, category) %>%
  group_by(category) %>%
  summarize(n = n()) %>%
  mutate(prop = (n/trips_remaining_in)*100)

trips_out_category <- trips_out %>%
  dplyr::filter(trip_id %in% good_trips) %>%
  distinct(trip_id, category) %>%
  group_by(category) %>%
  summarize(n = n()) %>%
  mutate(prop = (n/trips_remaining_out)*100)
```
