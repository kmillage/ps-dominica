...
shipping_lane_length <- 14*nm # This one doesn't need to be exact, just needs to be long enough to bisect the reserve
...
NE_coordinates_2 <- c(-61.458272, 15.638654) # Northernmost point on the island
SE_coordinates_2 <- c(-61.374291, 15.214981) # Tip of Scotts Head
...
NE_sf_2 <- data.frame(loc = "NE") %>%
  mutate(geom = st_sfc(st_point(NE_coordinates_2))) %>%
  st_as_sf() %>%
  st_set_crs(st_crs(latlon_crs)) # Keep in lat/lon from now

# Plot 
ggplot()+
  geom_sf(data = dominica_eez_sf_moll, aes(group = mrgid), color = "black", fill = "lightblue4", size = 0.2)+
  geom_sf(data = NE_sf_2)
```
...
```{r}
# Get mollewide points of NE coordinate
NE_coord_2 <- st_coordinates(NE_sf_2)

# Create container for NW points
NW_sf_2 <- data.frame(loc = paste0("NW_", W_longitudes, "deg"),
                      lon = W_longitudes) %>%
  rowwise() %>%
  mutate(geom = st_sfc(st_point(c(lon, NE_coord_2[2])))) %>%
  st_as_sf() %>%
...
ggplot()+
  geom_sf(data = dominica_eez_sf_moll, aes(group = mrgid), color = "black", fill = "lightblue4", size = 0.2)+
  geom_sf(data = NW_sf_2, aes(group = loc))
```
...
```{r}
# Set SE boundary. Coordinates eyeballed from map Enric sent.  
SE_sf_2 <- data.frame(loc = "SE") %>%
  mutate(geom = st_sfc(st_point(SE_coordinates_2))) %>%
  st_as_sf() %>%
  st_set_crs(st_crs(latlon_crs)) # Keep in lat/lon for now

# Plot 
ggplot()+
  geom_sf(data = dominica_eez_sf_moll, aes(group = mrgid), color = "black", fill = "lightblue4", size = 0.2)+
  geom_sf(data = SE_sf_2)
```
...
```{r}
# Get mollewide points of SE coordinate
SE_coord_2 <- st_coordinates(SE_sf_2)

# Create container for NW points
SW_sf_2 <- data.frame(loc = paste0("SW_", W_longitudes, "deg"),
                      lon = W_longitudes) %>%
  rowwise() %>%
  mutate(geom = st_sfc(st_point(c(lon, SE_coord_2[2])))) %>%
  st_as_sf() %>%
  st_set_crs(latlon_crs)
...
ggplot()+
  geom_sf(data = dominica_eez_sf_moll, aes(group = mrgid), color = "black", fill = "lightblue4", size = 0.2)+
  geom_sf(data = SW_sf_2, aes(group = loc))
```
...
```{r}
# Get corner coordinates 
NW_coord_2 <- st_coordinates(NW_sf_2)
SW_coord_2 <- st_coordinates(SW_sf_2)

# Create reserve polygons
reserve_sf_2 <- data.frame(scenario = seq(1, nrow(NW_coord_2)),
                              NW_coord_x = NW_coord_2[,1],
                              NW_coord_y = NW_coord_2[,2],
                              SW_coord_x = SW_coord_2[,1],
                              SW_coord_y = SW_coord_2[,2]) %>%
  rowwise() %>%
  mutate(geom = st_sfc(list(st_polygon(list(matrix(c(NE_coord_2,
                                         SE_coord_2,
...
ggplot()+
  geom_sf(data = dominica_eez_sf_moll, aes(group = mrgid), color = "black", fill = "lightblue4", size = 0.2)+
  geom_sf(data = reserve_sf_2, aes(group = scenario, fill = scenario), alpha = 0.5)
```

## Refine East Boundary

NOTE - If we have a high resolution map of Dominica, we can use that. I couldn't find one in the GD, so I'm doing a negative buffer on the shoreline from the Marine Regions EEZ boundary. 
...
reserve_sf_revised_2 <- reserve_sf_2 %>%
  st_transform(crs = st_crs(prj_moll)) %>%
...
ggplot()+
  geom_sf(data = dominica_eez_sf_moll, aes(group = mrgid), color = "black", fill = "lightblue4", size = 0.2)+
  geom_sf(data = reserve_sf_revised_2, aes(group = scenario, fill = scenario), alpha = 0.5)
```
## Add Shipping Channel

```{r}
# Subtract shipping channel and summarize
reserve_with_sl_sf_moll_2 <- st_difference(reserve_sf_revised_2, sl_sf_moll) %>%
  mutate(area_m2 = st_area(geom),
         area_km2 = as.numeric(area_m2)*1e-6,
         method = "Version 2 - Coord + Longitude + Shore Buffer",
         W_lon = paste0(W_longitudes, "deg")) %>%
  dplyr::select(method, scenario, W_lon, area_km2)
  
# Plot 
reserve_plot <- ggplot()+
  geom_sf(data = dominica_eez_sf_moll, aes(group = mrgid), color = "black", fill = "lightgrey", size = 0.2)+
  geom_sf(data = reserve_with_sl_sf_moll_2, aes(group = scenario, fill = scenario), alpha = 0.3)+
  geom_sf(data = NE_sf_2, size = 0.8)+
  geom_sf(data = SE_sf_2, size = 0.8)+
  theme_basic()+
  guides(fill = guide_colorbar(title = "Scenario", title.position = "top"))+
  coord_sf(xlim = plot_bbox$xlim, ylim = plot_bbox$ylim)
...
save(reserve_with_sl_sf_moll, file = file.path(here::here("results", "reserve_boundaries.Rdata")))
```
