...
reserve_plot_w_ferry <- ggplot()+
  geom_sf(data = dominica_eez_sf_moll, aes(group = mrgid), color = "black", fill = "grey", size = 0.2)+
  geom_sf(data = ferry_ais_positions, size = 0.2)+
  geom_sf(data = reserve_with_sl_sf_moll, aes(group = scenario, fill = scenario), alpha = 0.3)+
  theme_basic()+
  guides(fill = guide_colorbar(title = "Scenario", title.position = "top"))+
  coord_sf(xlim = plot_bbox$xlim, ylim = plot_bbox$ylim)
...
ferry_positions_by_buffer <- ferry_ais_positions_all %>%
  ...
  ggplot()+
  aes(x = fct_rev(`Shore buffer`), y = prop_positions_in_reserve*100)+
  geom_point()+
  theme_basic()+
...
ferry_hours_by_buffer <- ferry_ais_positions_all %>%
  ...
  mutate(`Shore buffer` = paste0(E_dist, " nm")) %>%
  ggplot()+
  aes(x = fct_rev(`Shore buffer`), y = prop_hours_in_reserve*100)+
  geom_point()+
  theme_basic()+
  labs(y = "% of ferry hours within reserve", x = "Shoreline buffer")+
  facet_wrap(~ year)
